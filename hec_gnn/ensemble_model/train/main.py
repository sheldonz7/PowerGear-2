...
class HECConvNet(nn.Module):
    def __init__(self, in_channels, hidden_channels, num_layers, dim, 
       ...
       # pdb.set_trace()
        ...
        # first layer
        self.convs.append(HECGATConv(in_channels, hidden_channels,dim,num_relations=relations,num_heads=num_heads, dropout=drop_out))
        ...
        for i in range(1, num_layers-1):
            ...
            self.convs.append(HECGATConv(hidden_channels * num_heads, hidden_channels,dim,num_relations=relations,num_heads=num_heads, dropout=drop_out))
            #self.bns.append(nn.BatchNorm1d(hidden_channels * num_heads))

        # output GAT layer
        self.convs.append(HECGATConv(hidden_channels * num_heads, hidden_channels,dim,num_relations=relations,num_heads=1, dropout=drop_out))
